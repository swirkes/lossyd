...
# Add the Oscillator class
class SineWaveOscillators:
    def __init__(self, server):
        self.server = server
        self.oscillators = [pyo.Sine(freq=1000, mul=0.1).out() for _ in range(25)]

    def update_oscillators(self, frequencies, db_values):
        print("Inside update_oscillators")  # Test print
        for osc, freq, db in zip(self.oscillators, frequencies, db_values):
            osc.freq = float(freq)
            osc.mul = (10 ** (float(db) / 20.0))
            print(f"Oscillator amplitude: {osc.mul}")
...
def process_hackrf(server):
    print("Starting process_hackrf")  # Test print
    osc_manager = SineWaveOscillators(server)
    ...
    for data in parse_hackrf_output(hackrf_process):
        ...
        print("Before osc_manager.update_oscillators")
        osc_manager.update_oscillators(frequencies, db_values)
        print("After osc_manager.update_oscillators")

if __name__ == '__main__':
    try:
        print("Script is starting")
        process_hackrf(server)
        server.gui(locals())
